...
int server_setup() {
  ...
  //?? check msg
  if((cts = open(CTS, O_RDONLY)) == -1) {
    printf("[server] create wkp fail\n");
  }
...
int server_connect(int from_client) {
  ...
  //make downstream pipe
  mkfifo(STC,0644);
  
  if((stc = open(STC, O_WRONLY)) == -1) {
    printf("[server] create downstream pipe fail\n");
  }
  
  printf("[server] created downstream pipe\n");
  //*to_client = stc;
  
  write(stc,STCM,sizeof(STCM));
  printf("[server] server to client message sent\n");
  
  read(from_client, tsm, sizeof(tsm));
